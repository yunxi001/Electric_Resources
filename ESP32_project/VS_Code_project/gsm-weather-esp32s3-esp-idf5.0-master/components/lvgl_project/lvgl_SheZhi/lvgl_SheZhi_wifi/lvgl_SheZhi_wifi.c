...
void lvgl_SheZhi_wifi_shuru_cb(char * text,void *dat)
{
	...
	if (flag == true)
	{
		...
		{
			lvgl_hint_create(lv_scr_act(), lvgl_globa_text[25][system_get_Language()], 120,3);
		}
	}
}
...
void lvgl_SheZhi_wifi_button_cb(struct _lv_obj_t * obj, lv_event_t event)
{
	...
	case LV_EVENT_CLICKED://非拖动下释放
		...
		if(lvgl_SheZhi_wifi_GongNeng_Data.wifi_dangqian_num == lvgl_SheZhi_wifi_GongNeng_Data.AnXia_Num)
		{
			...
			lvgl_SheZhi_wifi_GongNeng_Data.wifi_lianjie_flag = true;
			lvgl_hint_create(lv_scr_act(), lvgl_globa_text[26][system_get_Language()], 120,3);
		...
		if (1)//obj == lvgl_SheZhi_wifi_Button[0])
		{
			...
			sprintf(lvgl_SheZhi_wifi_GongNeng_Data.buf,"%s:%s",
				lvgl_globa_text[23][system_get_Language()],
...
//----标题----//
		...
		lv_style_copy(&SheZhi_wifi_ChuangKou.label_BiaoTi_style, &lvgl_font_ZiDing_K8_24_style);
...
//----标题----//
		...
		for (i = 0; i < SheZhi_wifi_Button_Num; i++)
		{
			...
			lv_style_set_text_font(&lvgl_SheZhi_wifi_Button_style[i], LV_STATE_DEFAULT, (const lv_font_t*)&font_ZiDing_K8_24);
	...
	lvgl_task_create(&lvgl_SheZhi_wifi_task,lvgl_SheZhi_wifi_TaskCb, 1000, LV_TASK_PRIO_HIGH, NULL);
...
void lvgl_SheZhi_wifi_ShuaXin_KaiShi(void)
{
	...
	lv_label_set_text(SheZhi_wifi_ChuangKou.label_BiaoTi, lvgl_globa_text[22][system_get_Language()]);
...
void lvgl_SheZhi_wifi_ShuaXin_jieshu(void)
{
	...
	lv_btn_set_state(SheZhi_wifi_ChuangKou.btn_BiaoTi, LV_BTN_STATE_RELEASED);
	lv_obj_set_pos(SheZhi_wifi_ChuangKou.label_BiaoTi, 0, 8);
	lv_label_set_text(SheZhi_wifi_ChuangKou.label_BiaoTi, lvgl_globa_text[21][system_get_Language()]);
