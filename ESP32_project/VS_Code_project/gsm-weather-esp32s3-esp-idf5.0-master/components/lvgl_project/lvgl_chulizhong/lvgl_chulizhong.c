...
void lvgl_chulizhong_create(lv_obj_t * Fu,char *tishi,uint32_t color, uint32_t anxia_color,void(*cb)(char * text, void *dat))
{
	...
	if (lvgl_chulizhong_main_cont == NULL)
	{
		...
		lv_style_set_bg_color(&lvgl_chulizhong_style, LV_STATE_DEFAULT, yangshi_moren_bg);//背景上面颜色
		lv_style_set_bg_grad_color(&lvgl_chulizhong_style, LV_STATE_DEFAULT, yangshi_moren_bg);//背景上面颜色
		...
		//lvgl_task_create(&lvgl_chulizhong_ChuangKou.lvgl_task,lvgl_chulizhong_TaskCb, 20, LV_TASK_PRIO_HIGH, NULL);
