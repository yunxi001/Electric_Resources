...
#if 1
  #define user_https_debug(format, ...)  my_debug("[user https]- ",format,##__VA_ARGS__);
#else
  #define user_https_debug(format, ...)  ;
#endif
...
int https_get_TianQi(void)
{
  ...
  if(system_data.City[0]==0)
  {
    ...
    if(res)
    {
      if(res == -3)
      {
        lvgl_TianQi_Set_ChengShi("定位城市返回空");
        user_https_debug("定位城市返回空\r\n");
      }else
      {
        lvgl_TianQi_Set_ChengShi("定位城市失败");
        user_https_debug("定位城市失败\r\n");
      }
      return -2;
    }
