...
{
    uint8_t res=0;
    res = i2c_master_write_read_device(I2C_PORT_NUM_TP, CST816T_ADDR, &reg_addr, 1, data, len, 100 / portTICK_PERIOD_MS);
    return res;
}
...
uint8_t cst816t_register_write_byte(uint8_t reg_addr, uint8_t data)
{
    ...
    ret = i2c_master_write_to_device(I2C_PORT_NUM_TP, CST816T_ADDR, write_buf, sizeof(write_buf), 100 / portTICK_PERIOD_MS);
    return ret;
}
